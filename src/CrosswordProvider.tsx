...
export const crosswordProviderPropTypes = {
  ...
  // A custom storage key to use for persistence; defaults to "guesses" when not provided
...
export interface CrosswordProviderImperative {
  ...
  reset: () => void;
  // Fills all the answers in the grid and calls the `onLoadedCorrect` callback with _**every**_ answer.
  fillAllAnswers: () => void;
  // Returns whether the crossword is entirely correct or not.
  isCrosswordComplete: () => boolean;
  // Returns whether the crossword is entirely correct or not. Sets the “guess” character for a specific grid position.
...
>(
  (
    ...
    // there seems to be a delay in 'focus' being usable after it's set.  We use a Ref instead.
    ...
    // This *internal* getCellData assumes that it's only ever asked for a valid cell (one that's used).
    ...
    const checkCorrectness = useCallback(
      (row: number, col: number) => {
        ...
        if (!cell.used) {
          ...
          // valid cell (row/col), the throw line isn't testable... so we ignore it.
    ...
    // Let the consumer know everything's correct (or not) if they've asked to be informed.
    useEffect(() => {
    ...
    const moveRelative = useCallback(
      (dRow: number, dCol: number) => {
        // We expect *only* one of dRow or dCol to have a non-zero value, and that's the direction
        // we will "prefer".  If *both* are set (or zero), we don't change the direction.
    ...
    >(
      (event) => {
        ...
        switch (key) {
          ...
          default:
            // It would be nice to handle "regular" characters with onInput, but that is still experimental,
            // so we can't count on it.  Instead, we assume that only "length 1" values are regular.
            if (key.length !== 1) {
    ...
    useEffect(() => {
      ...
      // handle bulkChange by updating a character at a time (this lets us leverage the existing character-entry logic).
    ...
    useEffect(() => {
      ...
      // Check all of the clues to see if any were correct... but only if we loaded guesses. 
      // Since the current implementation relies on state, we leverage the checkQueue to run
      // through all the clues/guesses.
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        // Returns whether the crossword is entirely correct or not.
        isCrosswordComplete: () => crosswordComplete,
